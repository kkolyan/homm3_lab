...
pub fn calculate_tournament(rounds: u32, crtrait0_txt: &str) {
    ...
    let total = creatures.len() * (creatures.len() - 1) * army_sizes.len() * dry_varians.len();
        ...
        .map(|tasks| {
            ...
            thread::spawn(move || {
                ...
                for (i, j, army_size, clean) in tasks {
                    let a = creatures.get(i).unwrap();
                    let b = creatures.get(j).unwrap();
    ...
    fs::write(format!("target/tournament.raw.{}.tsv", rounds), s).unwrap();
}

pub fn render(rounds: u32, structure_txt: &str) {
    let mut results: HashMap<String, Vec<FightResult>> = Default::default();
    let results_str = fs::read_to_string(format!("target/tournament.raw.{}.tsv", rounds)).unwrap();
