...
fn main() {
    ...
    for (i, castle) in structure.iter().enumerate() {
        ...
        table.add_column(castle.name.to_string(), move |double_level: &usize, w: &mut dyn fmt::Write| {
            ...
            write!(w, "{}", creature.cost.iter()
                .map(|(t, v)| match t {
                    ResourceType::Gold => *v,
                    ResourceType::Wood | ResourceType::Ore => v * 250,
                    ResourceType::Mercury | ResourceType::Sulfur | ResourceType::Crystal | ResourceType::Gems => v * 500,
                })
                .sum::<u32>())
        })
    }
    ...
    fs::write("target/cost_chart.tsv", s).unwrap();
}